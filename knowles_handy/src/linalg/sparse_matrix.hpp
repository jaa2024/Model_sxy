...
public:
  ...
  SparseMatrix(MatrixFillMode fill_mode, const std::vector<T> &values,
    ...
    descr.mode = (fill_mode == MatrixFillMode::UPPER) ? SPARSE_FILL_MODE_UPPER
                                                      : SPARSE_FILL_MODE_LOWER;
    descr.diag = SPARSE_DIAG_NON_UNIT;
    // 创建CSR格式稀疏矩阵（仅存储一半元素）
    sparse_status_t status = mkl_sparse_d_create_csr(
        &A, SPARSE_INDEX_BASE_ZERO, nrow_, ncol_, // 行数 == 列数
  ...
  ~SparseMatrix() { mkl_sparse_destroy(A); }
  ...
  std::vector<T> operator*(const std::vector<T> &x) const {
    ...
    if constexpr (std::is_same_v<T, double>) {
      sparse_status_t status =
          mkl_sparse_d_mv(SPARSE_OPERATION_NON_TRANSPOSE, alpha, A, descr,
                          x.data(), beta, y.data());
    } else if constexpr (std::is_same_v<T, std::complex<double>>) {
      sparse_status_t status =
          mkl_sparse_z_mv(SPARSE_OPERATION_NON_TRANSPOSE, alpha, A, descr,
                          x.data(), beta, y.data());
    }
    return y;
  }
  std::vector<T> sparse_v(const std::vector<T> &x) const {
    std::vector<T> y(nrow_, 0.0);
    const double alpha = 1.0;
    const double beta = 0.0;
    if constexpr (std::is_same_v<T, double>) {
